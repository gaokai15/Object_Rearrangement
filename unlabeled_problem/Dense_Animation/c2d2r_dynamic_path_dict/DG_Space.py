...
class Experiments(object):
    def single_instance(self, numObjs, RAD, HEIGHT, WIDTH, display, displayMore, savefile, saveimage, example_index):
        ...
        for i in range(numObjs):
            ...
            if group not in label_groups:
                label_groups[group] = []
            label_groups[group].append(i)
        
        # print label_groups

        labeling = Labeling(graph, label_groups, object_locations)
        best_labels = {}
        for i in range(numObjs):
            best_labels[labeling.best_labels[i][1]] = labeling.best_labels[i][0]
        ...
        for key in graph.keys():
            ...
            for i in key:
                ...
                try:
                    if (i > 0) and (i%2):
                        new_i = 2*best_labels[i//2] + 1
                except Exception:
                    pass
                new_key.append(new_i)
            new_key = tuple(new_key)
            new_graph[new_key] = []
            for reg in graph[key]:
                new_reg = []
                for i in reg:
                    new_i = i
                    try:
                        ...
                        pass
                    new_reg.append(new_i)
                new_reg = tuple(new_reg)
                new_graph[new_key].append(copy.deepcopy(new_reg))
        graph = copy.deepcopy(new_graph)
        new_graph = None

        new_paths = {}
        for key1, key2 in paths.keys():
            new_key1 = []
            for i in key1:
                new_i = i
                try:
                    ...
                    pass
                new_key1.append(new_i)
            new_key1 = tuple(new_key1)
            new_key2 = []
            for i in key2:
                new_i = i
                try:
                    ...
                    pass
                new_key2.append(new_i)
            new_key2 = tuple(new_key2)
            new_paths[(new_key1, new_key2)] = copy.deepcopy(paths[(key1, key2)])
        ...
        for i in range(len(objects)):
            if (i%2):
                new_objects[2*best_labels[i//2]+1] = copy.deepcopy(objects[i])
            else:
                new_objects[i] = copy.deepcopy(objects[i])
        objects = []
        for i in range(len(new_objects)):
            objects.append(copy.deepcopy(new_objects[i]))
        new_objects = None
        ...
        for i in range(len(color_pool)):
            for group in label_groups.values():
                if i in group:
                    new_pool.append(color_pool[group[0]])
        color_pool = copy.deepcopy(new_pool)
        new_pool = None

        new_points = {}
        for i in range(len(points)):
            if (i%2):
                new_points[2*best_labels[i//2]+1] = copy.deepcopy(points[i])
            else:
                new_points[i] = copy.deepcopy(points[i])
        points = []
        for i in range(len(new_points)):
            points.append(copy.deepcopy(new_points[i]))
        new_points = None

        # new_objectShape = {}
        # for i in range(len(objectShape)):
        #     print i
        #     if (i%2):
        #         new_objectShape[i] = copy.deepcopy(objectShape[2*best_labels[i//2]+1])
        #     else:
        #         new_objectShape[i] = copy.deepcopy(objectShape[i])
        # objectShape = []
        # for i in range(len(new_objectShape)):
        #     objectShape.append(new_objectShape[i])
        # new_objectShape = None
        ...
        for key in object_locations.keys():
            new_key = key
            if (key%2):
                new_key = 2*best_labels[key//2] + 1
            reg = object_locations[key]
            new_reg = []
            for i in reg:
                new_i = i
                try:
                ...
                new_reg.append(new_i)
            new_reg = tuple(new_reg)
            new_locations[new_key] = new_reg
        object_locations = copy.deepcopy(new_locations)
        new_locations = None
    ...
    def density_test(self, numObjs, RAD, HEIGHT, WIDTH, display, displayMore, savefile, saveimage, example_index):
        ...
        for key in Data.keys():
            print key
            print Data[key]
        with open(os.path.join(my_path, "DensityExperiments_n"+str(numObjs)+"_0713.pkl"), 'wb') as output:
...
class Labeling(object):
    def __init__(self, graph, label_groups, object_locations):
        n = 0
        for group in label_groups.values():
            n += len(group)
        self.obj_locations = object_locations
        self.linked_list_conversion(graph)
        self.assignment_graph = [[2*n+1 for j in range(n)] for i in range(n)]
        for group in label_groups.values():
            for start in group:
                start_pose = 2*start
                for goal in group:
                    goal_pose = 2*goal+1
                    cost = self.transformation(start, start_pose, goal_pose)
                    self.assignment_graph[start][goal] = cost
        #             print start, goal, cost
        # print self.assignment_graph
        
        m = Munkres()
        self.best_labels = m.compute(self.assignment_graph)

    def transformation(self, obj, start_pose, goal_pose):    
        Found = False
        parents = {}
        explored = {}
        for key in self.region_dict.values():
            explored[key] = 0
        queue = [self.region_dict[self.obj_locations[start_pose]]]
        explored[self.region_dict[self.obj_locations[start_pose]]] = 1
        if (self.region_dict[self.obj_locations[start_pose]]==self.region_dict[self.obj_locations[goal_pose]]):
            return 2
        while (len(queue) >0) and (not Found):
            ...
            if old_node in self.LL:
                for region in self.LL[old_node]:
                    ...
                    if region == self.region_dict[self.obj_locations[goal_pose]]:
                        Found = True
                        break
                    queue.append(region)
                    explored[region] = 1
        ...
        if Found:
            dep_set = set()
            current_node = self.region_dict[self.obj_locations[goal_pose]]
            ...
            return len(self.obj_locations)+1
