...
class Visualizer(object):
    def __init__(self, HEIGHT, WIDTH, numObjs, wall_pts, display, displayMore, saveimage, instance_dir):
        self.HEIGHT = HEIGHT
        self.WIDTH = WIDTH
    ...
    def updateDisplaySetting(self, display, displayMore, saveimage):
        self.display = display
        self.displayMore = displayMore
        self.saveimage = saveimage

    def addSolutionFolder(self, solution_subfolder, tree_subfolder):
    ...
    def drawProblem(self, objects, points, buffers=[]):
        ...
        if self.saveimage and (buffers == []):
            plt.savefig(self.instance_dir + "/original_problem.png")
        if self.saveimage and (buffers != []):
            plt.savefig(self.instance_dir + "/original_problem_with_buffers.png")
        if self.display:
            plt.show()
        plt.close()
        return
    ...
    def drawRegionGraph(self, paths, regions, label=True):
        ...
        if self.saveimage and paths == {}:
            plt.savefig(self.instance_dir + "/regionGraph.png")        
        if self.saveimage and paths != {}:
            plt.savefig(self.instance_dir + "/regionGraph_connection.png")
        if self.display:
            plt.show()
        plt.close()
        return
    ...
    def drawConGraph(self, paths, points, objects, buffers=[]):
        ...
        if self.saveimage:
            plt.savefig(self.instance_dir + "/cgraph.png")
        if self.display:
            plt.show()
        plt.close()
        return
