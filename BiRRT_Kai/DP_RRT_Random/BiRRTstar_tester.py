...
class BiRRTstar_tester(object):
    ...
    def __init__(self, initial_arrangement, final_arrangement, sample_arrangements, gpd, instance, new_paths, visualTool):
        ...
        while (self.isConnected != True and timeout > 0):     
            ...
            new_arrNode = self.treeExpansionL(self.sample_arrangements[timeout])
            ### connect to right tree
            self.connectToTreeR(new_arrNode)
            if (self.isConnected != True and timeout > 0):
                timeout -= 1
                ### expand the right tree
                new_arrNode = self.treeExpansionR(self.sample_arrangements[timeout])
    ...
    def treeExpansionL(self, sample_arrangement):
        ...
        if arc_setSize == 0:
            ...
            if shortest_dist > self.threshold:
                ...
                new_arrNode = ArrStarTestNode(partial_arrangement, "L"+str(self.left_idx),
                ...
                new_arrNode = ArrStarTestNode(new_arrangement, "L"+str(self.left_idx),
    ...
    def treeExpansionR(self, sample_arrangement):
        ...
        if arc_setSize == 0:
            ...
            if shortest_dist > self.threshold:
                ...
                new_arrNode = ArrStarTestNode(partial_arrangement, "R"+str(self.right_idx),
                    nearest_arrID, self.threshold + self.treeR[nearest_arrID].num_actions, partial_object_ordering, partial_path_selection)
            else:
                ### the distance between new_arrangement and nearest_arrID node is within the threshold
                new_arrNode = ArrStarTestNode(new_arrangement, "R"+str(self.right_idx),
