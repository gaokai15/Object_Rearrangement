...
class feedback_arc_ILP_buffers(object):
    ...
    def run_arc_ILP_buffers(self):
        ...
        m.setObjective(
            sum(
                sum(C[k, i] * y[k, i]
                    for k in range(i)) + sum(C[l, i] * y[l, i]
        ...
        m.addConstrs(
            C[j, k] <= sum(x[i, u] for (i, u) in dependencyEdge_paths[(j, k)]) for (j, k) in dependencyEdge_paths.keys()
        )
        for (j, k) in dependencyEdge_paths.keys():
            for (i, u) in dependencyEdge_paths[(j, k)]:
                m.addConstr(C[j, k] >= x[i, u])

        m.addConstrs(C[i, i] == 0 for i in range(n))
