...
from cgraph import genCGraph, loadCGraph, drawMotions, animatedMotions
...
class Experiments(object):
    def single_instance(self, numObjs, RAD, HEIGHT, WIDTH, display, displayMore, savefile, saveimage, example_index):
        ...
        # scaler = 1000.0
        graph, paths, objects, color_pool, points, objectShape = genCGraph(
        ...
        paths_opts = gpd.dependency_dict
        if display:
            rpaths = self.drawSolution(HEIGHT, WIDTH, numObjs, RAD, paths, paths_opts, path_selection, objects, color_pool, points, example_index, saveimage)
    ...
    def drawSolution(self, HEIGHT, WIDTH, numObjs, RAD, paths, path_opts, path_selection, objects, color_pool, points, example_index, saveimage):
        rpaths = OrderedDict()
        for obj, iopt in enumerate(path_selection, 1):
            dpath = path_opts[obj][iopt]
            # print(obj, iopt, dpath)
            deps = {2 * (x[0] - 1) + x[1] for x in dpath}
            start = 2 * obj - 2
            goal = 2 * obj - 1
            # print(deps)
            ...
            while deps:
                for ind in deps:
                    key = tuple(sorted([curInd, ind]))
                    if key in paths:
                        if min(curInd, ind) == curInd:
                            rpath += paths[key][:-1]
                        else:
                            rpath += list(reversed(paths[key]))[:-1]
                        curInd = ind
                        deps.remove(ind)
                        break
            key = tuple(sorted([curInd, goal]))
            if min(curInd, goal) == curInd:
                rpath += paths[key]
            else:
                rpath += list(reversed(paths[key]))
