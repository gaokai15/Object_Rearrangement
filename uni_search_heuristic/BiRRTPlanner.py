...
class BiRRTPlanner(object):
    ...
    ### Output:
    ### the whole plan
    def __init__(self, initial_arrangement, final_arrangement, instance, gpd, new_paths, visualTool):
        self.initial_arrangement = initial_arrangement
        self.final_arrangement = final_arrangement
        self.gpd = gpd
        self.dependency_dict = gpd.dependency_dict ### shallow copy
        self.path_dict = gpd.path_dict ### shallow copy
    ...
    def treeExpansionL(self):
        print("try to expand the left tree")
        ...
        if arc_setSize == 0:
            ...
            if shortest_dist > self.threshold:
                ...
                new_arrNode = ArrNode(partial_arrangement, "L"+str(self.left_idx), 
                ...
                new_arrNode = ArrNode(new_arrangement, "L"+str(self.left_idx), 
    ...
    def treeExpansionR(self):
        print("try to expand the right tree")
        new_arrangement = self.generateNewArrangement()
        ### (1) step1: Now find the nearest node in the left tree
        ...
        if arc_setSize == 0:
            ...
            if shortest_dist > self.threshold:
                ...
                new_arrNode = ArrNode(partial_arrangement, "R"+str(self.right_idx), 
                                nearest_arrID, self.threshold, partial_object_ordering, partial_path_selection)    
                print("partial arrangement: " + "R"+str(self.right_idx) + ": " + str(partial_arrangement))
                print("path selection: " + str(partial_path_selection))
                print("object ordering: " + str(partial_object_ordering))
            else:
                new_arrNode = ArrNode(new_arrangement, "R"+str(self.right_idx), 
                                    nearest_arrID, shortest_dist, object_ordering, path_selection)
    ...
    def connectToTreeR(self, query_node):
        print("try to connect to the right tree")
        ...
        print("time for trying to connect the two roots: " + str(time.clock() - start_time))
        IPython.embed()
        IP_arc_buffers = feedback_arc_ILP_buffers(object_dependency_opts)
        arc_setSize, arcs, path_selection, object_ordering = IP_arc_buffers.optimum
        print "ILP result(the smallest size of FAS with buffers):", IP_arc_buffers.optimum
    ...
    def connectToTreeL(self, query_node):
        print("try to connect to the left tree")
        ### in case the query node is None
        if query_node == None:
            return
    ...
    def object_dependency_opts_generate(self, query_arrangement, nearest_arrangement):
        ...
        for obj_idx in range(num_objs):
            # ### we only look at the objects which differs between query_arrangement and nearest_arrangement
            # if query_arrangement[obj_idx] == nearest_arrangement[obj_idx]:
            #     continue
            ...
            ### Now check if (pose_key1, pose_key2) has already been in the self.dependency_dict
            if (pose_key1, pose_key2) not in self.dependency_dict.keys():
                ### we need to generate these paths online
                self.gpd.update_dependency_path_dict(pose_key1, pose_key2, self.numObjs)
