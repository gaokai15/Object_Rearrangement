...
class Search:
    ...
    def perform_search(self):
        ...
        pq = priority_queue()
        pq.insert(
            self.roadmap.vertices_dict[0][0], self.roadmap.vertices_dict[0][1], self.g[0], 0
        )
        ...
        while not pq.isEmpty():
            ...
            if [temp.x, temp.y] == self.roadmap.vertices_dict[1]:
                final_path, final_path_idx = [
                    list(reversed(item)) for item in self.get_final_path()
                ]
            ...
            for node_idx in successors:
                if self.roadmap.vertices_dict[node_idx] not in closed_list:
                    ...
                    gTemp = self.g[temp.idx
                                   ] + self.roadmap.edge_weights[temp.idx][distance_index]
