...
class configuration_space:
    ...
    def fromParams(self, HEIGHT, WIDTH, objects, start=None, goal=None):
        self.boundary = [(0, 0), (WIDTH, 0), (WIDTH, HEIGHT), (0, HEIGHT)]
        self.polygons = objects
    ...
    def plot_polygon(self, coords):
        ...
        plt.plot(
            [elem[0] for elem in [coords[0], coords[-1]]],
            [elem[1] for elem in [coords[0], coords[-1]]],
            color='black'
        )
        for i in range(1, len(coords)):
            plt.plot(
                [elem[0] for elem in coords[i - 1:i + 1]],
                [elem[1] for elem in coords[i - 1:i + 1]],
                color='black'
            )
