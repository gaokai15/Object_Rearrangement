...
class Visualizer(object):
    ...
    def drawProblem(self, objects, points, buffers=[]):
        ...
        _, ax = self.setupPlot()
        ### plot the wall first
        wall_pt = self.wall_pts + [self.wall_pts[0]]
        wallx = [p[0] for p in wall_pt]
        wally = [p[1] for p in wall_pt]
    ...
    def setupPlot(self):
        ...
        fig = plt.figure(
            num=None, figsize=(int(5 * self.WIDTH / self.HEIGHT), 5), dpi=120, facecolor='w', edgecolor='k'
        )
        ax = fig.subplots()
        return fig, ax
